...
fn list_methods(db: Box<Database>) {
    ...
    let mut index = ProofListIndex::new(TABLE_NAME, &mut fork);
...
fn iter(db: Box<Database>) {
    let mut fork = db.fork();
    let mut list_index = ProofListIndex::new(TABLE_NAME, &mut fork);
...
fn same_root_hash(db1: Box<Database>, db2: Box<Database>) {
    ...
    let mut i1 = ProofListIndex::new(TABLE_NAME, &mut fork1);
    i1.push(vec![1]);
    i1.push(vec![2]);
    i1.push(vec![3]);
    i1.push(vec![4]);
    ...
    let mut i2 = ProofListIndex::new(TABLE_NAME, &mut fork2);
    i2.push(vec![4]);
    i2.push(vec![7]);
    i2.push(vec![5]);
    i2.push(vec![1]);
