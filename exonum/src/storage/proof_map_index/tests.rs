...
fn insert_trivial(db1: Box<Database>, db2: Box<Database>) {
    ...
    let mut index1 = ProofMapIndex::new(TABLE_NAME, &mut storage1);
    index1.put(&[255; 32], vec![1]);
    index1.put(&[254; 32], vec![2]);

    let mut index2 = ProofMapIndex::new(TABLE_NAME, &mut storage2);
...
fn insert_same_key(db: Box<Database>) {
    ...
    let mut table = ProofMapIndex::new(TABLE_NAME, &mut storage);
...
fn fuzz_delete_build_proofs(db: Box<Database>) {
    ...
    let mut rng = rand::thread_rng();
    let mut storage = db.fork();
    let mut index = ProofMapIndex::new(TABLE_NAME, &mut storage);
    for item in &data {
...
fn iter(db: Box<Database>) {
    ...
    let mut map_index = ProofMapIndex::new(TABLE_NAME, &mut fork);
